...
type TmProvinceController struct {
	TmProvinceService *services.TmProvinceService
}

func NewTmProvinceController(priceService *services.TmProvinceService) *TmProvinceController {
	return &TmProvinceController{
		priceService,
	}
}

func (r TmProvinceController) Get(c *fiber.Ctx) error {
	var data interface{}
	...
	qProvinceId := c.Params("id")
	if qProvinceId != "" {
		provinceId, err = strconv.Atoi(qProvinceId)
		if err != nil {
	...
	params := domain.ProvinceRequestParams{
		CommonParams:     common.CommonParams{Ctx: c},
		PaginationParams: paginationParams,
		Id:               provinceId,
	}

	data, errorDomain = r.TmProvinceService.Get(params)
	if errorDomain != nil {
		return common_helper.ProcessErrorDomain(c, errorDomain)
	}

	dataCount, errorDomain = r.TmProvinceService.Count(params)
	if errorDomain != nil {
		return common_helper.ProcessErrorDomain(c, errorDomain)
	}
...
func (r TmProvinceController) GetById(c *fiber.Ctx) error {
	var data interface{}
	var errorDomain *common.ErrorDomain
	var err error
	var provinceId int
	...
	qProvinceId := c.Params("id")
	if qProvinceId == "" {
		return common_helper.GenerateResponseJSON(c, http.StatusBadRequest, models.SetError().Details(http.StatusBadRequest, "provinceId is required", nil))
	}
	provinceId, err = strconv.Atoi(qProvinceId)
	if err != nil {
		return common_helper.GenerateResponseJSON(c, http.StatusInternalServerError, models.SetError().Details(http.StatusInternalServerError, errorDomain.Message, nil))
	...
	data, errorDomain = r.TmProvinceService.GetById(params)
	if errorDomain != nil {
		return common_helper.ProcessErrorDomain(c, errorDomain)
	}
