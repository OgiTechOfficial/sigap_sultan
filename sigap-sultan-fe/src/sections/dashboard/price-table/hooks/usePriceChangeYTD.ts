import { priceYTDApi } from "@/api/base/price/priceYTD";
...
function usePriceChangeYTD(
    ...
    useQuery({
      ...
      queryFn: async () => {
        const { result, error, displayMessage } =
          await priceYTDApi.getPriceYTDMap({
            page: 1,
            limit: 100,
            sortBy: "price:asc",
  ...
  const { data: dataPriceLevelList, isFetching: isFetchingPriceLevelList } =
    useQuery({
      ...
      queryFn: async () => {
        const { result, error, displayMessage } =
          await priceYTDApi.getPriceYTDList({
            page: 1,
            limit: 100,
        ...
        if (error || !result) {
          throw new Error(
            displayMessage ?? "Failed to fetch price change YTD list"
          );
        }
...
export default usePriceChangeYTD;
