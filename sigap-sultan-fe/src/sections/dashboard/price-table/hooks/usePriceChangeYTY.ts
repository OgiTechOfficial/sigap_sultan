...
import { priceYTYApi } from "@/api/base/price/priceYTY";
...
function usePriceChangeYTY(
    ...
    useQuery({
      ...
      queryFn: async () => {
        const { result, error, displayMessage } =
          await priceYTYApi.getPriceYTYMap({
            page: 1,
            limit: 100,
            sortBy: "price:asc",
  ...
  const { data: dataPriceLevelList, isFetching: isFetchingPriceLevelList } =
    useQuery({
      ...
      queryFn: async () => {
        const { result, error, displayMessage } =
          await priceYTYApi.getPriceYTYList({
            page: 1,
            limit: 100,
        ...
        if (error || !result) {
          throw new Error(
            displayMessage ?? "Failed to fetch price change YTY list"
          );
        }
...
export default usePriceChangeYTY;
