import { priceCompareNationalApi } from "@/api/base/price/priceCompareNational";
...
function useCompareNational(
    ...
    useQuery({
      ...
      queryFn: async () => {
        const { result, error, displayMessage } =
          await priceCompareNationalApi.getPriceCompareNationalMap({
            page: 1,
            limit: 100,
            sortBy: "price:asc",
  ...
  const { data: dataPriceLevelList, isFetching: isFetchingPriceLevelList } =
    useQuery({
      ...
      queryFn: async () => {
        const { result, error, displayMessage } =
          await priceCompareNationalApi.getPriceCompareNationalList({
            page: 1,
            limit: 100,
        ...
        if (error || !result) {
          throw new Error(
            displayMessage ?? "Failed to fetch compare national list"
          );
        }

        try {
          const pricePerCity = (result.cityPrice || []).map(
            (price, priceIndex) => ({
              cityId: price.city.id,
          ...
          if (result.provincePrice) {
            pricePerProvince = {
              ...
              priceDiff: result.provincePrice.priceDiff,
              priceType: "ABSOLUTE",
            };
          }
          let pricePerNational: PricePerCityCardData | null = null;
          if (result.nationalPrice) {
            pricePerNational = {
              cityId: result.nationalPrice.country.id,
              city: result.nationalPrice.country.name,
              cityImage: result.nationalPrice.country.assets?.assetsUrl,
              price: result.nationalPrice.price,
              priceType: "ABSOLUTE",
            };
          }
          return {
...
export default useCompareNational;
