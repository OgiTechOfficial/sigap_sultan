...
import CompareProvincePerCityChartModal from "./components/CompareProvincePerCityChartModal";
import CompareProvincePerCityTableModal from "./components/CompareProvincePerCityTableModal";
import { PricePerCityData } from "@/types/pricePerCityMap";
import { PriceTableState } from "./components/PriceTableFilter";
import useCompareSulsel from "@/sections/dashboard/price-table/hooks/useCompareSulsel";
...
function CompareSulsel(props: Props) {
  ...
  const {
    ...
    dataPriceLevelList,
    dataPriceLevelMap,
  } = useCompareSulsel(priceTableState, sortBy);
      ...
      <Grid>
        <Grid.Col span={{ sm: 12, md: 3 }}>
          ...
          <LoadingPageContainer isLoading={isLoadingPriceLevelMap}>
            ...
            />
            {selectedCity && (
              <CompareProvincePerCityChartModal
                opened={mapModalOpened}
                close={closeMapModal}
                title="Selisih dibandingkan Sulsel (Rp)"
        ...
        <Grid.Col span={{ sm: 12, md: 9 }}>
          ...
          {selectedStatus && (
            <CompareProvincePerCityTableModal
              ...
              headerLabel={"Perbedaan harga dengan Sulsel"}
              priceTableState={priceTableState}
