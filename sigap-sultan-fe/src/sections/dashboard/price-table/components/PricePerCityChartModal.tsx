...
import { PriceDiffBy, PriceListRequest, priceApi } from "@/api/base/price";
...
function PricePerCityChartModal(props: Props) {
  const {
    title,
    opened,
    close,
  ...
  const { data: dataPricePerCity, isFetching: isLoadingPricePerCity } =
    useQuery({
      queryKey: [
        "price-per-city-chart-price-level",
        selectedCity.city,
      ...
      queryFn: async () => {
        ...
        const { result, error, displayMessage } = await priceApi.getList({
...
export default PricePerCityChartModal;
