import {
  ...
  PriceCompareNationalCommodityHistoryRequest,
...
function CompareNationalPerCityTableModal(props: Props) {
  ...
  const { data: dataPricePerCity, isFetching: isLoadingPricePerCity } =
    useQuery({
      queryKey: [
        "price-per-city-table-compare-national",
      ...
      queryFn: async () => {
        const { result, error, displayMessage } =
          await priceCompareNationalApi.getPriceCompareNationalCommodityHistory(
            {
              ...
              selectedDate: priceTableState.date
                ...
                : "",
            } as PriceCompareNationalCommodityHistoryRequest
          );
...
export default CompareNationalPerCityTableModal;
