...
import {
  ...
  PriceCompareProvinceCityHistoryRequest,
...
function CompareProvincePerCityChartModal(props: Props) {
  const {
    title,
    opened,
    close,
  ...
  const { data: dataPricePerCity, isFetching: isLoadingPricePerCity } =
    useQuery({
      queryKey: [
        "price-per-city-chart-compare-province",
        selectedCity.city,
      ...
      queryFn: async () => {
        ...
        const { result, error, displayMessage } =
          await priceCompareProvinceApi.getPriceCompareProvinceCityHistory({
          ...
          } as PriceCompareProvinceCityHistoryRequest);
...
export default CompareProvincePerCityChartModal;
