...
function useLastStockRegion(neracaState: NeracaFilterState) {
  const { data: dataLastStockCityMap, isFetching: isLoadingLastStockCityMap } =
    useQuery({
      queryKey: [
        "region-last-stock-city-map",
      ...
      queryFn: async () => {
        const { result, error, displayMessage } =
          await neracaApi.getNeracaLastStockByCityMap({
            page: 1,
            limit: 100,
...
export default useLastStockRegion;
