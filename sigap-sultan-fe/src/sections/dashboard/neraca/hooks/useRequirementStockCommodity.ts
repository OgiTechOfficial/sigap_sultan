...
function useRequirementStockCommodity(
  neracaState: NeracaFilterState,
  sortBy: string | null
) {
  const {
    data: dataRequirementStockCommodityMap,
    isFetching: isLoadingRequirementStockCommodityMap,
  } = useQuery({
    queryKey: [
      "requirement-stock-commodity-map",
    ...
    queryFn: async () => {
      const { result, error, displayMessage } =
        await neracaRequirementApi.getNeracaRequirementStockByCommodityMap({
          page: 1,
          limit: 100,
          sortBy: "id:asc",
  ...
  const {
    data: dataRequirementStockCommodityList,
    isFetching: isLoadingRequirementStockCommodityList,
  } = useQuery({
    queryKey: [
      "requirement-stock-commodity-list",
      neracaState.commodityType,
      neracaState.date,
      sortBy,
      neracaState.requestTimestamp,
    ...
    queryFn: async () => {
      const { result, error, displayMessage } =
        await neracaRequirementApi.getNeracaRequirementStockByCommodityList({
          page: 1,
          limit: 100,
      ...
      if (error || !result) {
        throw new Error(
          displayMessage ?? "Failed to fetch requirement stock commodity"
        );
      }
...
export default useRequirementStockCommodity;
