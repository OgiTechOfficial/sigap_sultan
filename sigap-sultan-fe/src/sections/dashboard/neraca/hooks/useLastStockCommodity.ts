...
function useLastStockCommodity(
  neracaState: NeracaFilterState,
  sortBy: string | null
) {
  const {
    data: dataLastStockCommodityMap,
    isFetching: isLoadingLastStockCommodityMap,
  } = useQuery({
    queryKey: [
      "last-stock-commodity-map",
    ...
    queryFn: async () => {
      const { result, error, displayMessage } =
        await neracaApi.getNeracaLastStockByCommodityMap({
          page: 1,
          limit: 100,
          sortBy: "id:asc",
  ...
  const {
    data: dataLastStockCommodityList,
    isFetching: isLoadingLastStockCommodityList,
  } = useQuery({
    queryKey: [
      "last-stock-commodity-list",
      neracaState.commodityType,
      neracaState.date,
      sortBy,
      neracaState.requestTimestamp,
    ...
    queryFn: async () => {
      const { result, error, displayMessage } =
        await neracaApi.getNeracaLastStockByCommodityList({
          page: 1,
          limit: 100,
      ...
      if (error || !result) {
        throw new Error(
          displayMessage ?? "Failed to fetch last stock commodity list"
        );
      }
...
export default useLastStockCommodity;
