...
function useAvailabilityRegion(neracaState: NeracaFilterState) {
  const { data: dataLastStockCityMap, isFetching: isLoadingLastStockCityMap } =
    useQuery({
      queryKey: [
        "region-availability-city-map",
      ...
      queryFn: async () => {
        const { result, error, displayMessage } =
          await neracaAvailabilityApi.getNeracaAvailabilityByCityMap({
            page: 1,
            limit: 100,
...
export default useAvailabilityRegion;
