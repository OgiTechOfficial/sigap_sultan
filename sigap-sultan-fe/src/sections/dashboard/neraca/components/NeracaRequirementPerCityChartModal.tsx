...
function NeracaRequirementPerCityChartModal(props: Props) {
  ...
  const { data: dataPricePerCity, isFetching: isLoadingPricePerCity } =
    useQuery({
      queryKey: [
        "neraca-map-summary-requirement",
        selectedCity.city,
        timeFilter,
      ...
      queryFn: async () => {
        ...
        const { result, error, displayMessage } =
          await neracaRequirementApi.getNeracaRequirementCityHistory({
...
export default NeracaRequirementPerCityChartModal;
