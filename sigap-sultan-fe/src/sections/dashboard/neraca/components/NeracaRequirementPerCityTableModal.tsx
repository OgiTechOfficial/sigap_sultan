import {
  ...
  neracaRequirementApi,
} from "@/api/base/neraca/neracaRequirement";
...
function NeracaRequirementPerCityTableModal(props: Props) {
  ...
  const { data: dataStockPerCity, isFetching: isLoadingStockPerCity } =
    useQuery({
      queryKey: [
        "neraca-per-city-table-requirement",
        neracaState.city,
      ...
      queryFn: async () => {
        const { result, error, displayMessage } =
          await neracaRequirementApi.getNeracaRequirementCommodityHistory({
            ...
            <Table.Tbody>
              {dataStockPerCity?.cityStock?.map((data, index) => (
                <Table.Tr key={index}>
                  ...
                  <Table.Td>
                    <Text size="md">{FormatNumber(data.kebutuhan)}</Text>
                  </Table.Td>
                  <Table.Td>
                    {stockTier && (
...
export default NeracaRequirementPerCityTableModal;
